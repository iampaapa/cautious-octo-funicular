...
export const getQuestions = async (
  ...
  return await window.api.getQuestions(subject, limit, questionType)
}

export const saveSettings = (quizSettings: QuizSettings): void => {
  ...
  if (quizTypesParsed.find((quiz) => quiz.id === quizSettings.id)) {
    quizTypesParsed = quizTypesParsed.map((quiz) =>
      quiz.id === quizSettings.id ? quizSettings : quiz
    )
  } else {
...
export const deleteQuizSetting = (id: string): void => {
  const quizTypes = localStorage.getItem('quizTypes')
...
export async function generateAudio(text: string): Promise<void> {
  return await window.api.generateAudio(text)
}
